...
(function () {
	...
	// Ip
	Utils.Ip = {
		long2ip: function (ip_long) {
			var i = ip_long
			var iv = (ip) => (ip >>> 24)
			var result = [0, 8, 16, 24]
			return result.map(r => iv(i << r)).join('.')
		},
		ip2long: function (ip_str) {
			var type = Object.prototype.toString.call(ip_str)
			...
			var t = (prev, cur) => prev * Number(1 << 8) + cur
	...
	Utils.Cookies = {
		get: function (key, defaultValue = "") {
			return ((new RegExp(["(?:; )?", key, "=([^;]*);?"].join(""))).test(document.cookie) && RegExp["$1"]) || defaultValue;
		},

		set: function (key, value, expires, domain) {
			document.cookie = [key, "=", value, expires && expires["toGMTString"] ? ';expires=' + expires.toGMTString() : "", ";path=/;domain=", domain || ""].join("");
		},

		remove: function (key) {
			document.cookie = [key, "=", "", ';expires=' + new Date(new Date().getTime() - 99).toGMTString(), ";path=/;domain=", ""].join("");
		},
	...
	Utils.String = {
		...
		isNest: function (rule, str) {
			...
			var stack = [];
			for (var i = 0; i < len; i++) {
				if (Utils.Array.inArray(keys, chs[i])) {
					stack.push(rule[chs[i]]);
				} else {
					if (chs[i] === stack[stack.length - 1]) {
						stack.pop();
					} else if (Utils.Array.inArray(values, chs[i])) {
						return false;
					}
				}
			}
	...
	Utils.IdCard = {
		...
		getPersonInfo18: function (idCard) {
			...
			var address = '';
			var sex = '';
			address = Utils.areas[idCard.substr(0, 2) + '0000'] + ' ' + Utils.areas[idCard.substr(0, 4) + '00'] + ' ' + Utils.areas[idCard.substr(0, 6)];
	...
	Utils.Date = {
		...
		//获取当前时间
		getCurrentTime: function (preciseTime = true) {
			var today = new Date();
		...
		// 加天数
		addDay: function (dateObj, day) {
			var date = Date.parse(new Date(dateObj)) / 1000;//转换为时间戳
			date += 24 * 3600 * day;
			var after = new Date(parseInt(date) * 1000);//转换为时间
			return this.formt(after, "yyyy-MM-dd HH:mm:ss");
		}
	};
	// 简单四则运算
	Utils.ElementaryArithmetic = {
		//运算符优先级
		operatorPrecedence: {
			'+': 0,
			'-': 0,
		...
		infixToPostfixExpression: function (expression) {
			expression = Utils.String.deleteWhitespace(expression);
			...
			for (var i = 0; i < size; i++) {
				if (Utils.String.isNumeric(elementArr[i])) {
					//如果是数值
		...
		calculateByPrefixExpression: function (expression) {
			...
			for (var i = size - 1; i >= 0; i--) {
				if (Utils.String.isNumeric(elementArr[i])) {
					...
					switch (elementArr[i]) {
						case '+':
							result = Utils.ElementaryArithmetic.add(operand1, operand2);
							break;
						case '-':
							result = Utils.ElementaryArithmetic.subtract(operand1, operand2);
							break;
						case '×':
						case '*':
							result = Utils.ElementaryArithmetic.multiply(operand1, operand2);
							break;
						case '\/':
						case '÷':
							result = Utils.ElementaryArithmetic.divide(operand1, operand2);
							break;
						default:
							result = '';
		...
		calculateByPostfixExpression: function (expression) {
			...
			for (var i = 0; i < size; i++) {
				if (Utils.String.isNumeric(elementArr[i])) {
					...
					switch (elementArr[i]) {
						case '+':
							result = Utils.ElementaryArithmetic.add(operand2, operand1);
							break;
						case '-':
							result = Utils.ElementaryArithmetic.subtract(operand2, operand1);
							break;
						case '×':
						case '*':
							result = Utils.ElementaryArithmetic.multiply(operand2, operand1);
							break;
						case '\/':
						case '÷':
							result = Utils.ElementaryArithmetic.divide(operand2, operand1);
							break;
						default:
							result = '';
							alert("The operator 【" + elementArr[i] + "】 is not legal");
	...
	Utils.Regex = {
		...
		createRegexObjMustExclude: function (input, conditions) {
			...
			var subPattern = "";
			for (var i = 0; i < size; i++) {
				targetStrArr[i] = Utils.String.escapeMetacharacterOfStr(targetStrArr[i]);
				subPattern += targetStrArr[i];
				if (i != size - 1) {
					subPattern += "|";
				}
			}
			regex = Utils.String.format(regex, [subPattern]);
			switch (matcherFlag) {
				case '0':
					regex += "\\d";
					break;
			...
			regex += Utils.String.isNotBlank(length) ? "{" + length + "}" : "+";
			regex += "$";
